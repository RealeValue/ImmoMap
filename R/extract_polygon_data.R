...
#'
#' @noRd
extract_polygon_data_server <- function(id, action){
  moduleServer(id, function(input, output, session){
    ns <- session$ns

   polygon_data <- eventReactive(action(), {
      input_liste_hier <- reactiveValuesToList(input)
      input_liste_hier <<- input_liste_hier
     if( is.na(input$xco_wgs84) | is.na(input$yco_wgs84)) shiny::validate("Need Coordinates") else{

     df <- tibble(xco_wgs84 = input$xco_wgs84, yco_wgs84 = input$yco_wgs84) %>%
     ...
     if( is.na(df$bez_id)) shiny::validate("Need coordinates within Oesterreich")
     df
     }
   })
